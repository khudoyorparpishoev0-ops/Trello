/**
 * Чистые функции предметной области: даты, сроки, коды задач, чек-листы.
 *
 * Файл общий для фронтенда и сервера, поэтому здесь нет ничего, что тянет
 * браузер или Tailwind: `cn()` остался на стороне интерфейса, в
 * `src/lib/utils.ts`, и именно из-за него эта половина раньше не могла уехать
 * в общий код.
 */
