 ...
 * Единственный источник правды для токенов фронтенда HONA Core 2.0.
 * Скопировано из легаси tailwind.config.ts без изменения токенов (§23.3, KEEP);
 * изменён только путь CSS-переменных — src/index.css этого приложения.
 *
 * Поверхности и статусы заданы CSS-переменными в apps/web/src/index.css и переключаются
